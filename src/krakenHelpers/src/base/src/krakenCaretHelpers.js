...
export const krakenCaretHelpers = {
        "get": getCaretPosition,
        "set": setCaretPosition,
        "isFirstLine": isCaretOnFirstLine,
        "isLastLine": isCaretOnLastLine
}
...
export function setCaretPosition(element, value){
        ...
        if (element.textContent.length < value){
                value = element.textContent.length
        }

         let range = document.createRange();
            let sel = window.getSelection();
            range.setStart(element.childNodes[0], value);
            range.collapse(true);
